<template>
    ...
    <b-card header-tag="header" border-variant="light" style="max-width: 40rem;" class="mb-2 mx-auto">
      <h6 slot="header" class="mb-0"><b>Add new Hotel</b></h6>
        <form @submit="add" inline>
          ...
          <b-form-group>
            <b-form-input v-model="floors" placeholder="Number of floors" type="number"></b-form-input>
          </b-form-group>

          <b-form-group>
            <b-form-input v-model="rooms" placeholder="Max number of rooms" type="number"></b-form-input>
          </b-form-group>
  ...
  methods: {
    add (e) {
      ...
      const airline = {
        ...
        'floors': this.floors,
        'maxRoomsNum': this.rooms,
        'description': this.description
      }

      AXIOS.post('/hotels/add', airline)
      .then(response => {
        this.success = true;
        this.error = false;
      })
