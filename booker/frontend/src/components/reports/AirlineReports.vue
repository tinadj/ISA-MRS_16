        ...
        <b-col>
            <div class="control-section">
                ...
                <ejs-chart style='display:block' :theme='theme' align='center' id='chartcontainer' :title='title' :primaryXAxis='primaryXAxis' :primaryYAxis='primaryYAxis'
                    ...
                    <e-series-collection>
                        <e-series :dataSource='seriesData' type='Line' xName='x' yName='y' name='Number of reserved tickets' width=2 :marker='marker'> </e-series>
                    </e-series-collection>
                </ejs-chart>
                </div>
            </div>
    ...
    methods: {
        show: function(e) {
            ...
            if (this.dates != null) {
                ...
                // Daily resrved tickets
                if (this.reportType == 0) {
                    this.primaryXAxis.labelFormat = "d-M-y"
                    this.marker.visible = false

                    AXIOS.get("/flight-reservations/report-daily/" + this.$route.params.id + "/" + strStartDate + "/" + strEndDate)
                    .then( response => {
                ...
                // Weekly reserved tickets
                } else if (this.reportType == 1) {
                    this.primaryXAxis.labelFormat = "d-M-y"
                    this.marker.visible = true

                    AXIOS.get("/flight-reservations/report-weekly/" + this.$route.params.id + "/" + strStartDate + "/" + strEndDate)
                    .then( response => {
                        for (let i = 0; i < response.data.length; i++) {
                            this.seriesData.push(
                                {x: new Date(dateCounter.getFullYear(), dateCounter.getMonth(), dateCounter.getDate()), y: response.data[i]}
                ...
                // Monthly reserved tickets
                } else {
                    this.primaryXAxis.labelFormat = "M y"
                    this.marker.visible = true

                    AXIOS.get("/flight-reservations/report-monthly/" + this.$route.params.id + "/" + strStartDate + "/" + strEndDate)
                    .then( response => {
                        for (let i = 0; i < response.data.length; i++) {
                            this.seriesData.push(
                                {x: new Date(dateCounter.getFullYear(), dateCounter.getMonth(), dateCounter.getDate()), y: response.data[i]}
